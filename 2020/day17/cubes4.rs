...
fn parse_input() -> Map {
	...
	for (y, line) in text.trim().split("\n").enumerate() {
		for (x, chr) in line.chars().enumerate() {
			...
			map.insert(Coords { x: x as isize, y: y as isize, z: 0, w: 0 }, status);
			xmax = x;
		}
...
fn cycle(old: Map) -> Map {
	...
	for w in new.bounds.w.min..=new.bounds.w.max {
						...
						for delta_z in -1..=1 {
							for delta_y in -1..=1 {
								for delta_x in -1..=1 {
									...
									let coords = Coords { x: x + delta_x, y: y + delta_y, z: z + delta_z, w: w + delta_w };
